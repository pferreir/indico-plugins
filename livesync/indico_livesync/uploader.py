...
class Uploader(object):
    ...
    def run(self, records):
        ...
        for i, batch in enumerate(grouper(records, self.BATCH_SIZE, skip_missing=True), 1):
            ...
            try:
                for j, proc_batch in enumerate(grouper(process_records(batch), self.BATCH_SIZE, skip_missing=True)):
                    self.logger.info('{} uploading chunk #{} (batch {})'.format(self_name, j, i))
                    self.upload_records(proc_batch, from_queue=True)
            except Exception:
    ...
    def run_initial(self, events):
        ...
        for i, batch in enumerate(grouper(events, self.INITIAL_BATCH_SIZE, skip_missing=True), 1):
            ...
            for j, processed_batch in enumerate(grouper(process_records(batch), self.BATCH_SIZE, skip_missing=True)):
